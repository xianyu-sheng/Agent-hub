...
class StartDashboard:
    """Agent 启动仪表盘 — agent-hub start 的 TUI。

    验证所有 Agent 的就绪状态，以精美双栏布局展示，
    自动关闭（无需手动 Ctrl+C）返回交互界面。

    使用方式：
        dash = StartDashboard(console)
        dash.set_agents([...])  # [{name, icon, display, exe, ready, status, tasks}]
        dash.set_models([...])  # [{name, provider, default}]
        with dash.run():
            dash.render()
            time.sleep(3)  # 展示 3 秒后返回
    """
    ...
    def set_agents(self, agents: list[dict]) -> None:
        self.agents = agents
        self.ready_count = sum(1 for a in agents if a.get("ready"))
    ...
    def _build_layout(self) -> Layout:
        ...
        header = Panel(
            Align.center(
                ...
                f"[dim]v0.1.0 · 按需调用架构 · CLI Agent 在执行任务时自动拉起[/dim]"
            ),
            style="bold blue",
        )
        ...
        root["body"]["agents"].update(self._build_agents_panel())
        root["body"]["info"].update(self._build_info_panel())

        # Footer
        tip = self.footer_text or (
            f"✅ {self.ready_count}/{len(self.agents)} 个 Agent 就绪"
            if self.agents else "使用 agent register 注册 Agent"
        )
        root["footer"].update(
            Panel(Align.center(f"[bold green]{tip}[/bold green]"), style="green")
        )

        return root

    def _build_agents_panel(self) -> Panel:
        """构建 Agent 状态面板（左栏）— 卡片式布局。"""
        parts: list[str] = []

        if not self.agents:
            return Panel(
                Text("  (无外部 Agent)\n  agent register 注册", style="dim"),
                title="🤖 Agent 状态",
                border_style="cyan",
            )
        ...
        for a in self.agents:
            ...
            parts.append(
                f"[bold {color}]{status_icon}[/] {icon} [bold cyan]{name}[/bold cyan]  "
                f"[{color}]{status}[/{color}]"
            )
            parts.append(f"  [dim]{display}[/dim]")
            parts.append(f"  [dim]{exe} · {tasks} tasks[/dim]")
            parts.append("")

        if self.internal_count > 0:
            parts.append(
                f"[dim]🔄 agent-hub · {self.internal_count} internal tasks[/dim]"
            )
    ...
    def _build_info_panel(self) -> Panel:
        ...
        # 模型信息
        lines.append("[bold]📦 模型配置[/bold]")
        if self.models:
            for m in self.models:
                star = "⭐ " if m.get("default") else "  "
                prov = f" [dim]({m.get('provider', '')})[/dim]" if m.get("provider") else ""
                lines.append(f"  {star}[green]{m['name']}[/green]{prov}")
        else:
            lines.append("  [dim]未配置 · models add 添加[/dim]")

        lines.append("")
        lines.append(f"[bold]📊 系统统计[/bold]")
        lines.append(f"  Agent 总数: {len(self.agents) + self.internal_count}")
        lines.append(f"  外部 Agent: {len(self.agents)}")
        lines.append(f"  内部 Agent: {self.internal_count}")
        lines.append(f"  已配置模型: {len(self.models)}")

        lines.append("")
        lines.append(f"[bold]💡 快速开始[/bold]")
        lines.append(f"  直接输入任务描述即可工作")
        lines.append(f"  示例: [dim]分析项目代码质量[/dim]")
        lines.append(f"  [dim]help 查看更多命令[/dim]")
